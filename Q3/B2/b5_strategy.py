...
B5_CONFIG = dict(sectors=7, ring_radius=1000., proposals_per_source=3,
                 ...
                 adaptive_sectors=True, batching=True, movable_anchors=True,
                 reuse_coverage=True)
...
class B5Strategy(Q2Proposal):
    ...
    def route_points(self, start, centers):
    ...
    def estimate_route(self, start, centers):
        return path_length(start, self.route_points(start, centers))/5

    def adjust_backbone(self, info):
        if not self.b5_config['movable_anchors'] or not self.backbone:
            return
        centers = {c:m for c,(m,_) in info.items()}
        for j,(k,p) in enumerate(self.backbone):
            itinerary = self.route_points(self.position, centers)
            i = next(i for i,q in enumerate(itinerary) if np.linalg.norm(p-q)<1e-7)
            prev = self.position if i==0 else itinerary[i-1]
            nxt = itinerary[i+1] if i+1<len(itinerary) else None
            q = self.adjust(k, prev, nxt, p)
            if np.linalg.norm(p-q)<.1:
                continue
            old_cost = path_length(self.position, itinerary)
            self.backbone[j] = (k,q)
            new_cost = 5*self.estimate_route(self.position, centers)
            if new_cost > old_cost+1e-6:
                self.backbone[j] = (k,p)
            else:
                self.b5_stats['anchor_adjustments'] += 1

    def reusable_sectors(self):
        if not self.b5_config['reuse_coverage']:
            return []
        return [k for k,_ in self.backbone
                if np.max(np.linalg.norm(self.corners[k]-self.position,axis=1))<=999.99
                and np.min(self.corners[k]@self.position)>0]
    ...
    def metrics(self, c, p, power):
        ...
        # Eligibility depends on observation history, not only polygon bytes:
        # a redundant bearing may leave the polygon unchanged.
    ...
    def route_eligible(self, p, urgent=False):
        ...
        if self.sector(p) in (k, self.finishing_sector):
            return True
    ...
    def choose_local(self, info, urgent=None):
        ...
        if not plans:
            return None
        return max(plans, key=lambda p:(round(p['gain_s'], 6),
                                        -np.linalg.norm(p['point']-self.position)))
    ...
    def scan_backbone(self):
        ...
        # Newly discovered sources belong to the station just visited. Keep its
        # zone open while moving toward the next anchor, rather than abandoning it.
        self.finishing_sector = k
        self._sector_actions = 0
        self.b5_stats['backbone_scans'] += 1
        return dict(kind='scan', point=point, channels=[], sector=k, gain_s=None)

    def scan_reused(self, covered):
        point = self.position.copy()
        self.scan(point)
        self.backbone = [(k,p) for k,p in self.backbone if k not in covered]
        self.finishing_sector = self.sector(point)
        self._sector_actions = 0
        self.b5_stats['reused_scans'] += 1
        return dict(kind='scan_reused',point=point,channels=[],gain_s=None)
    ...
    def run(self):
        ...
        for macro in range(self.b5_config['max_macros']):
            ...
            if len(self.cleared | self.absent) == 20:
                break
            info = self.state_info()
            self.adjust_backbone(info)
            for c, (center, _) in info.items():
                if self.route_eligible(center):
                    self.wait_age[c] += 1
            eligible = [c for c in info if self.route_eligible(info[c][0])]
            urgent = max(eligible, key=lambda c:(self.wait_age[c],-c)) if eligible else None
            ...
            reusable = self.reusable_sectors()
            if reusable:
                plan = self.scan_reused(reusable)
                served = []
            elif self.backbone and self._sector_actions >= self.b5_config['sector_action_limit']:
                plan = self.scan_backbone()
                served = []
            else:
                plan = self.choose_local(info, urgent) if info else None
                # A guaranteed clear on the planned route has gain ~0: it costs
                # the same now as later. Requiring positive information gain here
                # wrongly postpones it and causes a second lap around the arena.
                threshold = -self.b5_config['min_gain_s'] if plan is not None and plan['kind']=='clear' else self.b5_config['min_gain_s']
                if plan is None or (self.backbone and urgent is None and plan['gain_s'] < threshold):
            ...
            if len(self.trace) == before:
                raise RuntimeError('B5 selected a stale action; stop rather than spin')
            # Real scans can serve known channels too, including newly discovered
            # ones. Failed clear coordinates are excluded from subsequent trials.
            touched = set(served)
            ...
            for c in touched:
                self.wait_age[c] = 0
            self.b5_stats['max_wait_seen'] = max(self.b5_stats['max_wait_seen'],
                max((self.wait_age[c] for c in info if c not in self.cleared), default=0))
