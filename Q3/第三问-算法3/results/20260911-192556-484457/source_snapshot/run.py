...
from bayes_tsp.timely import TimelyConfig, TimelyPolicy
from bayes_tsp.scout import ScoutConfig, ScoutPolicy
...
POLICIES = ('bayes-fast', 'bayes-scout', 'bayes-scout400', 'bayes-timely', 'bayes-timely-half')+REFINEMENT_POLICIES+SECTOR_POLICIES+('bayes-tsp4', 'bayes-service', 'bayes-joint', 'bayes-joint-probe', 'bayes-reviewed',
...
def make_policy(name, config):
    ...
    if name in ('bayes-scout', 'bayes-scout400'):
        return ScoutPolicy(ScoutConfig(**{**asdict(config), 'stable_routing': False},
                                       scout_radius=400. if name.endswith('400') else 250.))
    if name in ('bayes-timely', 'bayes-timely-half'):
        return TimelyPolicy(TimelyConfig(**{**asdict(config), 'stable_routing': False},
                                         future_discount=.5 if name.endswith('half') else 0.))
...
def main():
    ...
    parser.add_argument('--policy', choices=POLICIES, default='bayes-sector')
    parser.add_argument('--compare', nargs='+', choices=POLICIES, default=['baseline-ring', 'bayes-joint', 'bayes-sector'])
    ...
    if args.mode == 'official' and (not args.connect or not args.robot_id or args.policy not in
                                   ('bayes-tsp', 'bayes-efficient', 'bayes-route-scan', 'bayes-joint')+SECTOR_POLICIES):
