...
def main():
    ...
    parser.add_argument('--methods', nargs='+', choices=['latest', 'b2', 'validated', 'mec', 'q2', 'current'],
    ...
    from candidate import CURRENT_CONFIG
    ...
    summary = dict(status='PASS' if all(r['failure'] is None for r in rows) else 'FAIL',
        ...
        methods=metrics, configurations=dict(current=dict(base=LATEST_CONFIG, **Q2_CONFIG, **CURRENT_CONFIG), latest=LATEST_CONFIG, validated=VALIDATED_CONFIG,
