...
class World(base.World):
    def __init__(self, sources, seed=0, error_mode='iid', **kwargs):
        ...
        for s in sources:
            if s.heading_deg is not None and (
                not math.isfinite(s.heading_deg) or not 0 <= s.heading_deg < 360
            ):
    ...
    def feedback(self, action):
        ...
        if source.heading_deg is not None:
            ...
            tolerance = 8 * np.finfo(float).eps * max(1.0, math.hypot(dx, dy))
...
def generate_world(seed, n=None, scenario='uniform', radius=None, error_mode='iid', directional_count=None):
    ...
    sources = [
        Source(
            int(c), (float(r * math.cos(a)), float(r * math.sin(a))), float(rad), float(h) if i < nd else None
        )
