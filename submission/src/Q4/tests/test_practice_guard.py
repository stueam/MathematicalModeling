...
def test_count_only_after_practice_exit():
    ...
    assert (
        practice.result_count(
            screen(state='测试已结束') + [{'name': '本次演练测试干扰源数量'}, {'name': '12'}]
        )
        == 12
    )
...
def test_type_counts_are_evaluation_only_and_must_agree(omni, directional):
    ...
    assert practice.result_composition(items) == {
        'omnidirectional_count': omni,
...
def test_webview_panes_retried_but_formal_still_rejected(monkeypatch):
    replies = iter(
        [json.dumps([{'name': 'Web 内容', 'type': 'ControlType.Pane'}]), json.dumps(screen('问题4正式测试'))]
    )
...
def test_serial_practice_loop_against_local_protocol_with_retry(tmp_path, monkeypatch):
    ...
    def ui():
        return (
            screen(state='测试已结束') + [{'name': '本次演练测试干扰源数量'}, {'name': '10'}]
            if stage['exited']
            else screen()
        )
    ...
    monkeypatch.setattr(practice.time, 'sleep', lambda *a: None)
    result = practice.run_once(tmp_path, CASE, Config())
    assert result['policy'] == 's21-probes'
    assert result['official_result_verified'] and result['cleared_count'] == 10
    assert result['max_cost_difference_s'] < 0.0001
