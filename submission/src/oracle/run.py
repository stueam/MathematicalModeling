...
def run(args):
    ...
    route_source = args.router.resolve()
    ...
    hashes = {str(route_source.relative_to(ROOT)): hashlib.sha256(route_source.read_bytes()).hexdigest(),
              ...
              'selfcheck.py': hashlib.sha256((ROOT / 'selfcheck.py').read_bytes()).hexdigest()}
    config = {'worlds': args.samples, 'workers': args.workers, 'seed': args.seed,
              ...
              'official_N_probability_sensitivity': None,
    ...
    try:
        ...
        summary = {'model': config, 'all_worlds_completed': True, 'verification': checks,
                   ...
                   'official_frequency_N_sensitivity': None,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--router', type=Path,
                        default=ROOT / 'open_routes.py',
                        help='Exact OpenRoutes implementation; use the archived snapshot for standalone reproduction')
