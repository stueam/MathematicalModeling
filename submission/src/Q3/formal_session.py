...
def redact_log(value, robot_id):
    """Redact identity fields without changing coordinates, times or request IDs."""
    if isinstance(value, dict):
        result = {}
        for key, item in value.items():
            if key == 'robot_id':
                result[key] = '<已隐去>'
            elif key in ('body', 'error') and isinstance(item, str):
                try:
                    body = json.loads(item)
                except ValueError:
                    # Labelled identities only; never bare digits or request IDs.
                    pattern = (
                        r"""((?:\brobot_id\b|队号)["']?\s*[:=]?\s*["']?)""" + re.escape(robot_id) + r'(?!\d)'
                    )
                    result[key] = re.sub(pattern, lambda match: match[1] + '<已隐去>', item)
                else:
                    safe = redact_log(body, robot_id)
                    result[key] = (
                        item
                        if safe == body
                        else json.dumps(json_safe(safe), ensure_ascii=False, allow_nan=False)
                    )
            else:
                result[key] = redact_log(item, robot_id)
        return result
    if isinstance(value, (list, tuple)):
        return [redact_log(item, robot_id) for item in value]
    return value
...
def identity(items, question, port, awaiting=True):
    ...
    if awaiting:
        if '尚未进入' not in names or not any(
            re.search(r'接口.*就绪|等待机器狗进入|等待进入', s) for s in names
        ):
...
class LoggedSession(requests.Session):
    ...
    def record(self, value):
        ...
        append(self.folder / 'http-attempts.jsonl', redact_log(value, self.robot_id))
...
def validate_action(action):
    if (
        action.kind not in ('measure', 'clear')
        or type(action.channel) is not int
        or not 1 <= action.channel <= 20
    ):
...
def run_once(folder, expected_case, question, build, inspect_ui, port=2026, max_actions=6000):
    ...
    dump(
        ...
        dict(
            ...
            implementation=getattr(policy, 'implementation', type(policy).__name__),
    ...
    summary = dict(
        ...
        official_all_cleared_verified=None,
        source_count=None,
        clearance_fraction=None,
        cleared_count=0,
        virtual_time_s=0.0,
        average_per_cleared_s=None,
        actions=0,
        error=None,
        warnings=[],
        upload_status='not_checked',
        official_log_exported=False,
    ...
    entry_stamp = None
    exit_stamp = None
    costs = dict(move_s=0.0, switch_s=0.0, measure_s=0.0, clear_success_s=0.0, clear_failure_s=0.0)
    max_error = 0.0
    try:
        ...
        if not (
            ...
            and 0 < remaining <= 1200
            and math.isfinite(limit)
            and 0 < limit <= 360000
            and entry['virtual_time_s'] == 0
        ):
        ...
        belief.deadline, belief.virtual_limit, belief.virtual_time = client.deadline - 5, limit, 0.0
        reason = 'action_limit'
        for index in range(max_actions):
            ...
            append(
                folder / 'actions.jsonl',
                dict(request_id=request_id, action=asdict(action), response=response, planning_s=planning),
            )
            summary['actions'] += 1
            ...
            max_error = max(
                max_error, abs(round(belief.virtual_time + move + switch + operation, 6) - last_virtual)
            )
            costs['move_s'] += move
            ...
            if (index + 1) % 50 == 0:
                print(
                    json.dumps(
                        dict(case=case, actions=index + 1, cleared=len(cleared), virtual_time_s=last_virtual)
                    ),
                    flush=True,
                )
        if belief.done():
        ...
        if time.monotonic() < client.deadline and last_virtual < limit:
            ...
            if (
                response.get('exit_reason') != 'user_exit'
                or not math.isfinite(exit_virtual)
                or abs(exit_virtual - last_virtual) > 1e-4
            ):
                raise RuntimeError('Unexpected exit response')
            summary['exited'] = True
            exit_stamp = response.get('real_timestamp_ms')
        summary['status'] = (
            'completed' if reason == 'algorithm_complete' and summary['exited'] else 'incomplete'
        )
    except BaseException as exc:
        summary.update(status='stopped', error=f'{type(exc).__name__}: {exc}')
    finally:
        summary.update(
            algorithm_complete=belief.done(),
            cleared_count=len(cleared),
            ...
            server_program_time_s=(exit_stamp - entry_stamp) / 1000
            if entry_stamp is not None and exit_stamp is not None
            else None,
            max_cost_difference_s=max_error,
            costs=costs,
            movement_m=5 * costs['move_s'],
            ended_at_utc=datetime.now(timezone.utc).isoformat(),
            completion_evidence=dict(
                belief_done=belief.done(),
                channel_status={str(c): p.status for c, p in belief.channels.items()},
            ),
        )
        dump(folder / 'summary.json', summary)
        safe_log = redact_log(client.log, robot_id)
    ...
    print(
        json.dumps(
            dict(
                ...
                cleared=len(cleared),
                average_per_cleared_s=summary['average_per_cleared_s'],
                output=str(folder),
            ),
            ensure_ascii=False,
        ),
...
def main(question, root, build, inspect_ui, max_actions):
    parser = argparse.ArgumentParser(
        description=f'Q{question} formal ONE session; default reads UI only. Never starts a test.'
    )
    ...
    with process_lock(args.port):
        ...
        files = [
            ...
            and not any(
                s in p.relative_to(root).parts for s in ('results', 'evidence', 'tests', '__pycache__')
            )
        ]
        dump(
            folder / 'config.json',
