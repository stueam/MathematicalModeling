...
from bayes_tsp.stations import adjust_station, covers_vertices
from bayes_tsp.shared import Action, GeometryError, Observation, core, disk, load
...
def census(known=False):
    b = CoupledBelief()
    for c in range(1, 21):
        result = 'direction' if known and c == 1 else 'no_signal'
        b.channels[c].update(reading((0.0, 0.0), result, 0.0 if result == 'direction' else None, c))
    return b


@pytest.mark.parametrize('reverse', [False, True])
def test_fixed_radius_cut_rejects_a_position_that_disk_exclusion_keeps(reverse):
    records = [reading((0.0, 0.0), 'direction', 0.0), reading((1000.0, 1000.0), 'no_signal')]
    if reverse:
        records.reverse()
    ...
    assert new.region.covers(Point(500, 0))
    assert new.coupling_cuts > 0


def test_halfplane_retains_equality_with_margin_and_is_translation_stable():
    for shift in (np.array([0.0, 0.0]), np.array([1000000.0, -1000000.0])):
        region = box(shift[0] - 1, shift[1] - 1, shift[0] + 1500, shift[1] + 1)
        clipped = receive_halfplane(region, shift, shift + 1000)
        assert clipped.covers(Point(shift + np.array([1000.0, 0.0])))
        assert not clipped.covers(Point(shift + np.array([1400.0, 0.0])))


def test_coupling_is_transactional_and_respects_rejections_and_same_id():
    b = CoupledBelief()
    a = Action('measure', (0.0, 0.0), 1)
    r = {'accepted': True, 'measure_result': 'direction', 'svd_deg': 0.0, 'virtual_time_s': 5.0}
    b.apply(a, r, 'one')
    ...
    with pytest.raises(GeometryError):
        b.apply(a, {'accepted': True, 'measure_result': 'no_signal', 'virtual_time_s': 10.0}, 'bad')
    assert b.channels[1].region.wkb == before
    assert len(b.channels[1].history) == 1 and b.steps == 1 and (b.virtual_time == 5.0)
...
def test_coupled_regions_keep_true_positions_under_legal_feedback(error_mode):
    sim = load('simulator')
    for target in ((500.0, 0.0), (1499.0, 0.0), (-700.0, 800.0)):
        for radius in (1000.0, 1500.0):
...
def test_station_adjustment_shortens_both_legs_and_verifies_feasibility(monkeypatch):
    ...
    original = (0.0, 0.0)
    pos, record = adjust_station(vertices, original, (500.0, 100.0), (500.0, -100.0))
    assert record['accepted'] and record['saved_leg_m'] > 800
    assert covers_vertices(vertices, pos)
    import bayes_tsp.stations as module

    monkeypatch.setattr(
        module,
        'minimize',
        lambda *a, **_kwargs: SimpleNamespace(success=False, x=np.array([1000000000.0, 1000000000.0]), nit=1),
    )
    pos, record = adjust_station(vertices, original, (500.0, 100.0), (500.0, -100.0))
...
def test_clear_service_executes_actual_channel_measurements():
    ...
    a = policy.choose(b)
    assert a.kind == 'clear'
    b.apply(a, {'accepted': True, 'clear_result': 'success', 'virtual_time_s': 5.0}, 'clear')
    a = policy.choose(b)
