...
def radius_likelihood(history, xy):
    ...
    """
    lo, hi = np.full(len(xy), 1000.0), np.full(len(xy), 1500.0)
    angular = np.ones(len(xy))
    valid = np.linalg.norm(xy, axis=1) <= 1800.0
    seen = set()
    for obs in history:
        ...
        if obs.result == 'direction':
            ...
            overlap = np.maximum(0, np.minimum(1, diff + 0.005) - np.maximum(-1, diff - 0.005))
            angular *= overlap / 0.01
...
class Posterior:
    ...
    def best_clear_point(self):
        ...
        probabilities = (
            np.linalg.norm(points[:, None, :] - self.xy[None, :, :], axis=2) <= 20
        ) @ self.weights
...
class BayesModel:
    def __init__(self, resolution=24, p0=0.65):
    ...
    def posterior(self, channel):
        ...
        key = tuple((o.action.kind, o.action.position, o.result, o.bearing) for o in channel.history)
        ...
        for resolution in (self.resolution, 2 * self.resolution):
            ...
            if valid.any():
                ...
                result = Posterior(xy, weights, lo, hi, weights @ xy, evidence, len(cells), resolution)
    ...
    def existence(self, belief):
        ...
        q = []
        for c in unknown:
            evidence = min(1.0, self.posterior(belief.channels[c]).evidence)
...
def finish_cost(point, xy, weights):
    ...
    return (
        float(np.linalg.norm(mean - point)) / 5
        + 3
        + 2 * chance
        + 11 * (1 - chance)
        + float(weights @ np.where(hit, 0, np.maximum(d - 18, 0))) / 5
    )
...
def clear_finish_cost(posterior, point):
    ...
    return (
        ...
        + 2 * chance
        + 11 * (1 - chance)
        + float(posterior.weights @ np.where(hit, 0, np.maximum(d - 18, 0))) / 5
    )


def expected_after_measure(posterior, point, bin_width=1.0):
    ...
    overlap = (
        np.maximum(
            0,
            np.minimum(upper[:, None], (indices + 1) * bin_width)
            - np.maximum(lower[:, None], indices * bin_width),
        )
        / 2
    )
    ...
    for axis in (0, 1):
        means[active, axis] = (
            np.bincount(ids, mass * nodes[:, axis], minlength=nbin)[active] / probability[active]
        )
    spread = np.linalg.norm(nodes - means[ids], axis=1)
    hit_mass = np.bincount(ids, mass * (spread <= 20), minlength=nbin)
    correction = np.bincount(
        ids, mass * np.where(spread <= 20, 0, np.maximum(spread - 18, 0)), minlength=nbin
    )
    costs = (
        probability * np.linalg.norm(means - point, axis=1) / 5
        + 3 * probability
        + 2 * hit_mass
        + 11 * (probability - hit_mass)
        + correction / 5
    )
    ...
    return cost, {
        'near': p_near,
        'no_signal': p_none,
        'direction': float(probability.sum()),
