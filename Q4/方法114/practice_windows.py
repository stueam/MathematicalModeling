...
def inspect_ui():
    for attempt in range(3):
        ...
        if any(i.get('type') != 'ControlType.Pane' for i in items):
            return items
        if attempt < 2:
            time.sleep(.5)  # WebView accessibility may initially expose only two panes.
...
def start_practice(resume=False):
    ...
    while time.monotonic() < deadline:
        items = inspect_ui()
        if has_title(items, 'test-run-title') and any(re.search(r'接口.*就绪|等待机器狗进入|等待进入', i['name']) for i in items):
            practice_identity(items, awaiting=True)
            return items
        time.sleep(1)
...
def result_composition(items):
    ...
    names = [i['name'] for i in items]
    matches = [re.search(r'本次案例含干扰源(\d+)个，其中全向(\d+)个、定向(\d+)个', s)
               for s in names]
...
class LoggedSession(requests.Session):
    ...
    def record(self, data):
        ...
        with self.path.open('a', encoding='utf-8') as log:
            log.write(text.replace(self.robot_id, '<参赛队号已隐去>')+'\n')
            log.flush()
    ...
    def post(self, url, **kwargs):
        ...
        try:
            ...
            self.record({'event': 'request_error', 'request_id': payload['request_id'],
                         'error': f'{type(exc).__name__}: {exc}', 'elapsed_s': time.monotonic()-started})
            raise
        self.record({'event': 'response', 'request_id': payload['request_id'],
...
def run_once(folder, expected_case, config, policy_name='mobile'):
    ...
    try:
        ...
        if not (math.isfinite(remaining) and 5 < remaining <= 1200 and
                math.isfinite(virtual_limit) and 0 < virtual_limit <= 360000 and initial_time == 0):
            raise RuntimeError('Unexpected entry time budgets or nonzero initial virtual clock')
        summary['server_deadline_utc'] = (datetime.now(timezone.utc)+timedelta(
            seconds=client.deadline-time.monotonic())).isoformat()
        dump(folder/'summary.json', summary)
        ...
        print(json.dumps({'case': case, 'entered': True, 'mode': 'problem4_practice_only',
                          'remaining_real_s': response['remaining_real_duration_s']}, ensure_ascii=False), flush=True)
        for index in range(6000):
            ...
            row = {'request_id': request_id, 'action': asdict(action), 'response': response,
                   'planning_s': planning, 'move_m': distance(belief.position, action.position)}
            rows.append(row)
            ...
            if (index+1) % 50 == 0:
                print(json.dumps({'case': case, 'actions': index+1, 'cleared': len(belief.cleared),
                                  'virtual_time_s': belief.virtual_time}, ensure_ascii=False), flush=True)
        if not belief.done():
        ...
        summary.update(certified_complete=belief.done(), cleared_count=len(belief.cleared),
            virtual_time_s=float(rows[-1]['response']['virtual_time_s']) if rows else belief.virtual_time,
            real_time_s=time.monotonic()-started, actions=len(rows), costs=costs,
            movement_m=5*costs['move_s'], measure_count=int(costs['measure_s']/5),
            switch_count=int(costs['switch_s']), clear_failures=int(costs['clear_failure_s']/3),
            max_cost_difference_s=max_cost_difference, belief_implementation=type(belief).__name__)
        summary.update(planning_time_s=sum(r['planning_s'] for r in rows),
                       counters=policy.counters,
                       channel_status={c: p.status for c, p in belief.channels.items()})
        summary['average_per_cleared_s'] = summary['virtual_time_s']/len(belief.cleared) if belief.cleared else None
        for entry in client.log:
            if 'request' in entry:
                entry['request']['robot_id'] = '<参赛队号已隐去>'
        dump(folder/'http-log.json', client.log)
        dump(folder/'summary.json', summary)
...
def main():
    ...
    parser.add_argument('--policy', choices=('mobile', 'trim', 'adaptive', 'probes'), default='mobile')
