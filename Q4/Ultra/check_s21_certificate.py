...
def verify(path):
    d=json.loads(Path(path).read_text(encoding='utf-8'));info=d['info'];scale=int(info['scale'])
    points=info['points'];require(len(points)==21,'wrong station count')
    ...
    seen=set();outside=0;max_sq=0
    stack=[(-2048*scale,-2048*scale,2048*scale,2048*scale,0)]
    while stack:
        x0,y0,x1,y1,depth=stack.pop();key=(x0,y0,x1,y1)
        dx=max(x0,0,-x1);dy=max(y0,0,-y1)
        if dx*dx+dy*dy>(1800*scale)**2:outside+=1;continue
        if key in leaves:
            item=leaves[key];require(item[4]==depth,'depth mismatch')
            h=[S[i] for i in item[5]];require(len(h)>=3,'insufficient hull points')
            ...
            for i,a in enumerate(h):
                ...
                for c in corners:
                    ...
                    require(sq<(999*scale)**2,'distance bound failed');max_sq=max(max_sq,sq)
            seen.add(key)
        else:
    ...
    return {'passed':True,'verified_cells':len(seen),'outside_cells':outside,
            'integer_checks':True,'max_support_corner_distance_m':max_sq**0.5/scale,
            'boundary_covered':True}

if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('certificate',nargs='?',default=str(Path(__file__).parent/'proofs'/'s21_certificate.json'))
    ap.add_argument('--output');a=ap.parse_args();r=verify(a.certificate)
