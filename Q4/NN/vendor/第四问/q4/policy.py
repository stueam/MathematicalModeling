...
class Policy:
    ...
    def eligible_tasks(self, b, goals, route):
        """Scheduling hook; every goal still contributes to the complete suffix."""
        return set(goals)
    ...
    def choose(self, b):
        ...
        eligible = self.eligible_tasks(b, goals, route)
        shortlist = list(dict.fromkeys([k for k in route if k in eligible][:3]
            + sorted((c for c in detected if c in eligible), key=lambda c: distance(b.position, goals[c]))[:2]))
        candidates = []
        for k in shortlist:
