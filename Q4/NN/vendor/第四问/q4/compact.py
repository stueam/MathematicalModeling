...
def make_belief(mode):
    ...
    if mode in ('bilateral-bound', 'bilateral-probes', 'bilateral-pair', 'bilateral-direct'):
        from .bilateral import BilateralChannel
        return Belief(channels={c: BilateralChannel() for c in range(1, 21)})
    if mode=='hull':
...
def make_policy(mode, config=None, **options):
    if mode in ('bilateral-pair', 'bilateral-direct'):
        from .probe_pairs import PairProbePolicy
        return PairProbePolicy(config, direct=mode == 'bilateral-direct')
    if mode in ('capacity2', 'capacity4', 'capacity6'):
        from .capacity import CapacityPolicy
        return CapacityPolicy(int(mode[-1]), config)
    if mode == 'shaped':
        from .shaped import ShapedPolicy
        return ShapedPolicy(config)
    if mode in ('bilateral-bound', 'bilateral-probes'):
        from .bilateral import BilateralProbePolicy
        return BilateralProbePolicy(config, add_candidates=mode == 'bilateral-probes')
    if mode in ('probes-tong22', 'probes-s21', 'probes-s25', 'probes-tri25'):
        from .repo_layouts import RepoLayoutPolicy
        return RepoLayoutPolicy(mode.removeprefix('probes-'), config)
