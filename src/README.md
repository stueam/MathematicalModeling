...
`verify.py` 检查运行模块可达性、Ruff 静态规则与格式、未使用代码、单元测试、Q2 两个完整网格搜索、Q3/Q4 完整本地任务、S21 自检与无参数证书入口、入口帮助。完整仓库还检查当前 Q3 结果的论文案例导入以及历史论文表格；单独复制 `src` 时记录并跳过这两项集成检查。测试不能使闲置的运行模块通过可达性检查。
