...
def inspect_ui():
    for attempt in range(3):
        ...
        if items:
            return items
        if attempt < 2:
            time.sleep(.3)  # Retry only a blank accessibility snapshot, not a rejected mode.
...
def has_title(items, identifier):
    ...
    return bool(titles) and all(t == '问题3演练测试' for t in titles)


def practice_identity(items, awaiting=False):
    if not has_title(items, 'test-run-title'):
        raise RuntimeError('Not a verified problem-three PRACTICE screen')
...
def click_practice(name):
    if name not in ('开始问题3演练测试', '返回演练测试'):
        raise ValueError('Only the two explicit problem-three PRACTICE controls are allowed')
    guard = 'practice-3-title' if name.startswith('开始') else 'test-run-title'
    script = r'''
$g=$root.FindFirst([System.Windows.Automation.TreeScope]::Descendants,[System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::AutomationIdProperty,'GUARD'))
if (!$g -or (($g.Current.Name -replace '\s','') -ne '问题3演练测试')) {throw 'Practice screen guard failed'}
...
def start_practice(resume=False):
    ...
    if resume:
        ...
        return items
    if has_title(items, 'test-run-title'):
...
def run_once(folder, expected_case, config):
    ...
    try:
        ...
        print(json.dumps({'case': case, 'entered': True, 'mode': 'problem3_practice_only',
        ...
        for index in range(4000):
            ...
            row = {'request_id': request_id, 'action': asdict(action), 'response': response, 'planning_s': planning}
            rows.append(row)
