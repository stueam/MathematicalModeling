...
def receive_halfplane(region, positive, negative):
    ...
    scale = max(float(np.abs(a).max()), float(np.abs(b).max()), float(np.abs(vertices).max()), 1.0)
    ...
    lo, hi = float(along.min()) - 1.0, float(along.max()) + 1.0
    depth = max(1.0, float(-signed.min()) + 2.0)
    allowed = Polygon(
        [
            anchor + lo * tangent,
            anchor + hi * tangent,
            anchor + hi * tangent - depth * n,
            anchor + lo * tangent - depth * n,
        ]
    )
...
class CoupledChannel(core.Channel):
    ...
    coupling_cuts: int = 0
    coupling_area_removed_m2: float = 0.0
    ...
    def update(self, obs):
        ...
        if candidate.status == 'detected' and obs not in self.history:
            if obs.result == 'direction':
                pairs = [
                    (obs.action.position, old.action.position)
                    for old in candidate.history
                    if old.result == 'no_signal'
                ]
            elif obs.result == 'no_signal':
                pairs = [
                    (old.action.position, obs.action.position)
                    for old in candidate.history
                    if old.result == 'direction'
                ]
            else:
                pairs = []
            for positive, negative in pairs:
                ...
                if not candidate.region.equals(before):
                    ...
                    candidate.coupling_area_removed_m2 += max(0.0, before.area - candidate.region.area)
