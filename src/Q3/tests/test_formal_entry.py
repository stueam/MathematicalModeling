...
def screen(title=None, code=CASE):
    return [{'id': 'test-run-title', 'name': title or f'问题{QUESTION} 正式 测试'},
...
class MiniPolicy:
    ...
    def choose(self, belief):
        return Action('clear' if belief.virtual_time else 'measure', (0., 0.), 1)
...
def install_transport(monkeypatch, fault=None, remaining=1200):
    state = dict(paths=[], action_posts=[], virtual=0.)
    ...
    def post(session, url, **kwargs):
        ...
        if kind == 'enter':
            return response(dict(accepted=True, virtual_time_s=0, real_timestamp_ms=1000,
                                 remaining_real_duration_s=remaining, max_virtual_duration_s=360000))
        if kind == 'exit':
            return response(dict(accepted=True, virtual_time_s=state['virtual'],
                                 exit_reason='user_exit', real_timestamp_ms=2000))
...
@pytest.mark.parametrize('title', ['问题3演练测试', '问题4演练测试', f'问题{7-QUESTION}正式测试', '正式测试'])
...
def test_default_is_read_only_even_on_ready_page(tmp_path, monkeypatch, capsys):
    ...
    assert formal.main(QUESTION, tmp_path, lambda: pytest.fail('Default must not build or connect'), screen, 10) == 0
...
def test_single_session_result_without_truth_count(tmp_path, monkeypatch):
    ...
    assert result['source_count'] is result['clearance_fraction'] is result['official_all_cleared_verified'] is None
...
def test_real_core_sequence_unchanged_with_lost_response_and_no_truth_ui(tmp_path, monkeypatch):
    ...
    def fake_post(session, url, **kwargs):
        ...
        if kind == 'enter':
            return response(dict(accepted=True, virtual_time_s=0, remaining_real_duration_s=1200, max_virtual_duration_s=360000))
        if kind == 'exit':
