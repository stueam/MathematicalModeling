...
from audit_practice import repair_segment
from bayes_tsp.refinement import RefinementConfig, RefinementPolicy, repaired_station
...
def test_failed_clear_prefers_new_parallax_measurement():
    ...
    p = RefinementPolicy(RefinementConfig(resolution=8))
...
@pytest.mark.parametrize('sectors', [6, 7, 8])
def test_refinement_finishes_without_rng_or_false_absence(monkeypatch, sectors):
    sim = load('simulator')
    world = sim.generate_world(534, 10, 'uniform', 1000., 'extreme')
    env, b = sim.LocalSimulator(world), CoupledBelief()
    policy = RefinementPolicy(RefinementConfig(resolution=8, sectors=sectors))
