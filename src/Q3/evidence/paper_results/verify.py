...
paper=(root/'essay/essay.tex').read_text(encoding='utf-8')
def close(a,b):
...
for name,count in [('random',40),('stress',24)]:
    ...
    assert len({(r['seed'],r['scenario'],r['error_mode'],r['n_input'],r['radius_input']) for r in rows})==count
    assert all(r['policy']=='bayes-fast' and r['all_cleared'] and r['certified_complete'] and r['error'] is None and r['source_count']==r['cleared_count'] for r in rows)
    for r in rows:
...
print('Verified 64 archived cases, cost accounting, oracle mean, and paper values.')
