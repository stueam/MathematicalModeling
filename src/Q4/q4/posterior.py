...
def illuminated_fraction(alpha, left, right):
    ...
    for shift in (-math.tau, 0., math.tau):
        lo, hi = alpha[:, None]-math.pi/2+shift, alpha[:, None]+math.pi/2+shift
        total += np.maximum(0., np.minimum(right, hi)-np.maximum(left, lo))
    return np.clip(total/np.maximum(right-left, 1e-300), 0., 1.)


def history_atoms(history, xy, directional_prior=.5):
    ...
    for o in history:
        ...
        if o.result == 'direction':
            ...
            angular *= np.maximum(0., np.minimum(1., diff+.005)-np.maximum(-1., diff-.005))/.01
        elif o.result == 'near':
            valid &= distance <= 5
    ...
    if measure:
        ...
        edges = np.sort(np.concatenate(((bearings-math.pi/2) % math.tau,
                                       (bearings+math.pi/2) % math.tau), axis=1), axis=1)
        ...
        lower = np.maximum(1000., np.max(distances[:, positive], axis=1)) if positive.any() else np.full(n, 1000.)
        allowed = np.all(lit[:, :, positive], axis=2)
        upper = np.minimum(1500., np.min(np.where(lit[:, :, ~positive], distances[:, None, ~positive],
                                                  np.inf), axis=2)) if (~positive).any() else np.full_like(left, 1500.)
        omni_upper = np.minimum(1500., np.min(distances[:, ~positive], axis=1)) if (~positive).any() else np.full(n, 1500.)
    else:
        left, right = np.zeros((n, 1)), np.full((n, 1), math.tau)
        allowed = np.ones((n, 1), dtype=bool)
        lower, omni_upper = np.full(n, 1000.), np.full(n, 1500.)
        upper = np.full_like(left, 1500.)
    atom_hi = np.column_stack((omni_upper, upper))
    fractions = np.column_stack((np.full(n, 1-directional_prior),
                                 directional_prior*(right-left)/math.tau*allowed))
    mass = np.maximum(atom_hi-lower[:, None], 0.)/500*fractions
...
class Posterior:
    ...
    def reception(self, point):
        ...
        radial = np.clip((self.upper-np.maximum(self.lower[:, None], d[:, None]))/
                         np.maximum(span, 1e-300), 0., 1.)
...
class Model:
    def __init__(self, resolution=16, directional_prior=.5, existence_prior=.65):
        ...
        self.resolution, self.directional_prior, self.existence_prior = resolution, directional_prior, existence_prior
        self.cache = {}
    ...
    def posterior(self, channel):
        ...
        for resolution in (self.resolution, 2*self.resolution):
            ...
            total = float(raw.sum())
            if total > 0:
                post = Posterior(xy[good], raw[good]/total, lower[good], upper[good], left[good], right[good],
    ...
    def existence(self, belief):
        ...
        q = []
        for c in unknown:
            e = min(1., self.posterior(belief.channels[c]).evidence)
...
def count_marginals(probabilities, known):
    ...
    return np.asarray([p*distribution(q[:i]+q[i+1:])[max(0, low-1):high].sum()/denominator
...
def finish_proxy(point, xy, weights):
    ...
    hit = d <= 20
    p = float(weights @ hit)
    recovery = float(weights @ np.where(hit, 0., d))
    return float(np.linalg.norm(mean-point))/5+3+2*p+12*(1-p)+2*recovery/5


def expected_measure(post, point, bin_deg=2.):
    ...
    missed = np.maximum(0., weights-received)
    ...
    return future, {'near_probability': p_near, 'no_signal_probability': p_none,
