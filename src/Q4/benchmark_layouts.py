...
def execute(job, output):
    ...
    def factory(mode, config):
        p = original_factory('probes', config)
...
def main():
    ...
    output = args.output or ROOT / 'results' / ('paired-' + datetime.now().strftime('%Y%m%d-%H%M%S-%f'))
    ...
    for i, scene in enumerate(('uniform', 'boundary', 'outward', 'tangent', 'cluster', 'near', 'backside')):
        configs += [
            (9100 + 2 * i, scene, 'extreme', 10, 1000.0, 10, 'stress'),
            (9101 + 2 * i, scene, 'correlated', 16, 1500.0, 16, 'stress'),
        ]
    jobs = [
        dict(
            seed=s,
    ...
    baseline.dump(
        ...
        dict(
            jobs=jobs,
            code_sha256=baseline.manifest(),
            s21_certificate=certificate,
            s21_model_domain_passed=True,
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = [pool.submit(execute, j, output) for j in jobs]
        for f in as_completed(futures):
            row = f.result()
            rows.append(row)
            baseline.dump(output / 'summary.json', rows)
            print(
                f'{len(rows)}/{len(jobs)} {row["case_id"]} {row["cleared_count"]}/{row["source_count"]} {row["virtual_time_s"]:.2f}s {row["stop_reason"]}',
                flush=True,
            )
    baseline.dump(output / 'aggregate.json', baseline.aggregate(rows))
    raise SystemExit(
        any(row['error'] or not row['all_cleared'] or not row['certified_complete'] for row in rows)
    )
