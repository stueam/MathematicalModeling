...
class Design:
    def __init__(self, p1=(-700., -250.), theta=25., reception=1200., nr=80, na=16, arc=128):
        self.p1 = np.asarray(p1, float)
        self.theta, self.eps = np.deg2rad([theta, 1.])
        self.reception, self.arc = reception, arc
        self.domain = Point(0, 0).buffer(1800, quad_segs=arc)
        self.p1_region = sector(self.p1, self.theta, self.eps, 5., reception).intersection(self.domain)
        if self.p1_region.area <= 0:
            raise ValueError('First observation has zero feasible area')
        radial = np.sqrt(np.linspace(25., reception**2, nr+1))
        angular = np.linspace(self.theta-self.eps, self.theta+self.eps, na+1)
        points, weights = [], []
        for i in range(nr):
            for j in range(na):
                cell = sector(self.p1, (angular[j]+angular[j+1])/2,
                              (angular[j+1]-angular[j])/2, radial[i], radial[i+1],
                              max(1, 128//na)).intersection(self.domain)
                if cell.area <= 1e-12:
                    continue
                c = cell.centroid
    ...
    def probabilities(self, q, width):
        ...
        overlap = np.maximum(0., np.minimum(right[:, None], (idx+1)*h)-np.maximum(left[:, None], idx*h))
    ...
    def evaluate(self, q, width=.5):
        q = np.asarray(q, float)
        if np.linalg.norm(q-self.p1) < 1e-9:
            return dict(x=float(q[0]), y=float(q[1]), J=self.initial_radius,
                        ED=self.initial_diameter, p_near=0., p_none=0., probability_sum=1.)
        ...
        rn, dn = metrics(self.p1_region.intersection(close)) if pn else (0., 0.)
        r0, d0 = metrics(self.p1_region.difference(receiving)) if p0 else (0., 0.)
        jr, jd = pn*rn+p0*r0, pn*dn+p0*d0
        for b in np.flatnonzero(probs > 0):
            region = normal.intersection(wedge(q, (b+.5)*h, self.eps+h/2))
            if region.is_empty:
                raise ArithmeticError('Positive probability with empty support; refine geometry')
            r, d = metrics(region)
            jr += probs[b]*r
            jd += probs[b]*d
        return dict(x=float(q[0]), y=float(q[1]), J=float(jr), ED=float(jd),
                    p_near=pn, p_none=p0, probability_sum=float(probs.sum()+pn+p0))
