...
25 T0=T4+T_K;
26 elseif s<344.5
27 T0=(25-T4)/(344.5-339.5)*(s-339.5)+T4+T_K;D1
28 else
2230 T0=25+T_K;
end
31 end
...
```matlab
for k=1e-6:1e-8:3e-6%1.67e-6 for h=14.4580%14.4:0.001:14.6
...
29 M(1,2)=-k/delta_x;
30 M(size_x,size_x-1)=-k/delta_x;
...
N(1)=h*T0; -
N(size_x)=h*T0; for index x=2:size x-1
N(index_x)=-A*(T(index_x+1,index_t-1)+T(index_x-1,index_t-1))..
+(2*A-1)*T(index_x,index_t-1);
end
T(:,index_t)=M\N;
...
11 1b=[165 185 225 245 65]; [165 1852费
12 ub=[185 205 245 265 100];13 [x,fval]=ga(@evaluate,5,[],[],[],[],1b,ub,[],opt);
14 toc;
...
7 8 T_K=273.15；%温度单位转换 delta_t=0.1；%单位:s
10 % 模型计算
11 tic;
...
18 toc;
19 %x=[169.4105 185.0242 225.3092 265.0000 85.6735];
20 %作图
21 global T1
22 global T2
23 global T3
...
46 T4=x(4);
47 v=x(5)；%速度
48 T_K=273.15；%温度单位转换
49 50 delta_t=0.1；%单位:s
...
89 peak=T\_sensor(peak\_index(1):index)-217;
...
4 X=csvread('pop.csv');
5 figure(1)
67 8 x1im([9.3,10.5]); xlabe1('面积(K*s)');
9 ylabel('对称性误差(K)')
10 hold on
