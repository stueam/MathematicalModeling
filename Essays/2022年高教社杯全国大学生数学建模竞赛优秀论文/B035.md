...
for (int i = 0; i <= 9; ++i) tmp[i] = p[i];   
double ans = 1e100;   
pair<int, int> s = make\_pair(-1, -1); 生在线   
for (int i = 2; i <= 9; ++i) {   
vector<int> drone;   
drone.push\_back(0); drone.push\_back(1); drone. push\_back(i);   
for (int j = 2; j <= 9; ++j) {   
...
```cpp
2 #include<bits/stdc++.h> #include "geometry.hpp" 大学生在
3 using namespace std; gov.cn
4
