...
\subsection{问题三}
源的数量和位置未知，因此任务既包括清除已发现源，也包括确认未知频道中没有遗漏。
检测会改变定位范围和后续访问位置，清除停留点又可能承担其他频道的搜索任务，
故选点与路线需要共同安排。我们以几何约束保留可能位置，用Bayes更新预测后续反馈，
比较检测、清除和覆盖搜索的预计完成时间，并在每次反馈后调整路线。
任务是否完成由成功清除记录与频道排除证据共同判断，最后通过清除比例、时间分解和策略对照评价效果。
\subsection{问题四}
···
...
问题二为单个干扰源选择下一检测点，问题三还要决定先处理哪个源，以及何时搜索尚未发现信号的频道。
检测会改变定位范围，清除会减少待处理的源，而未知频道的搜索又可能带来新的定位任务。
因此，待访问的位置和访问次序都需要随反馈更新，不能在任务开始时一次确定。

我们将剩余工作分为\textbf{已发现源的定位清除}和\textbf{未知频道的覆盖搜索}两类。
几何约束保留与观测相容的位置，贝叶斯更新估计各种反馈的概率，路线规划则将两类任务安排到同一次行程中。
每轮比较候选动作的预计完成时间，只执行下一步，再根据真实反馈重新决策。
...
局部费用的具体计算式及经验参数见附录~\ref{app:q3-local-cost}。
...
我们将这一过程实现为 \texttt{bayes-fast}，采用表~\ref{tab:q3-parameters} 的参数进行位置积分、反馈预测和路线计算。
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{0.96\linewidth}{lX}
    ...
    路线求解 & 不超过 $12$ 点时用精确动态规划，否则用 2-opt 等局部改进 \\
...
平均定位清除时间按总虚拟时间除以总清除源数计算，汇总时保留未完成案例，避免只看成功局的耗时。
...
\begin{samepage}
这些指标反映实际任务的完成情况。简单分析可知，单次任务中机器狗的移动耗时通常占比最高。为进一步给移动耗时提供参照，我们尝试考虑极端情况下——源位置全部已知——需要走多远。
...
\clearpage
\appendix
\section{问题三局部时间评分的实现细节}
\label{app:q3-local-cost}

本附录给出第5.3.3节局部费用的计算方法，以说明程序如何比较候选动作。
以下费用用于预测和排序，实际执行耗时仍按题设计时规则累计。
记机器狗当前位置为 $\boldsymbol p_t$、当前频道为 $c_t$，候选执行位置为 $\boldsymbol q$，目标频道为 $c$。

\textbf{局部费用衡量处理一个源还需多久。}
给定位置节点及其后验权重 $f=\{(\boldsymbol z_\ell,w_\ell)\}$，
令 $d_\ell=\|\boldsymbol z_\ell-\boldsymbol q\|$，
$p_f(\boldsymbol q)=\sum_{d_\ell\leq20}w_\ell$ 为在 $\boldsymbol q$ 清除成功的概率。
当前实现用下式近似到达该点后的清除及失败恢复费用：
\begin{equation}
  H_f(\boldsymbol q)=3+2p_f(\boldsymbol q)
    +11[1-p_f(\boldsymbol q)]
    +\frac{1}{5}\sum_{d_\ell>20}w_\ell(d_\ell-18).
  \label{eq:q3-clear-cost}
\end{equation}
前两项为本次光学定位和成功时的激光清除时间；后两项估计失败后的恢复操作及继续接近源的时间。
其中 $11\,\mathrm s$ 为恢复操作的经验合计，$18\,\mathrm m$ 为接近距离的近似扣减量，
均只用于动作评分，实际计时仍按式~\eqref{eq:q3-time} 执行。
若位置后验的均值为 $\boldsymbol\mu_f$，从测点出发继续处理该源的局部费用估计为
\[
  F(\boldsymbol q,f)=\frac{\|\boldsymbol q-\boldsymbol\mu_f\|}{5}
                    +H_f(\boldsymbol\mu_f).
\]

据此，尝试清除的局部评分为移动时间加 $H_f(\boldsymbol q)$；
补测的局部评分还需计入检测、必要的切频，以及各反馈下的后续费用：
\[
  C_{\mathrm{meas}}(\boldsymbol q,c)
  =\frac{\|\boldsymbol q-\boldsymbol p_t\|}{5}
   +5+\mathbf1_{\{c\ne c_t\}}
   +\sum_y\pi_y(\boldsymbol q)F(\boldsymbol q,f_y).
\]
其中 $f_y$ 为预测反馈 $y$ 对应的位置后验；距离过近时，可直接清除，该项后续费用取 $5\,\mathrm s$。
正常示向和无信号均进入预测，反馈概率和后验按前述模型计算。
这样，第二问中用于比较定位范围的反馈预测，在本问中转而比较后续时间。

\textbf{经验参数与总评分的衔接。}
式~\eqref{eq:q3-clear-cost} 中的 $11\,\mathrm s$ 和 $18\,\mathrm m$ 是当前实现采用的经验近似，
不由题设直接推出，也未据此声称取值最优。其适用性需要结合相同场景下的策略对照与参数敏感性结果评价。
尝试清除时，以到点移动时间加 $H_f(\boldsymbol q)$ 作为正文的局部费用；
补测时，以 $C_{\mathrm{meas}}(\boldsymbol q,c)$ 作为局部费用。
其余任务的预计路程与尚未计入的操作时间，再按式~\eqref{eq:q3-score} 加入总评分。
