...
\subsubsection{联合路线与停留点复用}

路线中的访问任务包括已发现源的代表点和未知频道的覆盖测站。
尚未充分定位的源以位置后验均值作为代表点；具备保证清除条件时，使用相应的安全清除点。
这些位置随观测更新，不是预先固定的目标坐标。

对于未知频道，将其剩余可能区域按角向分区组织为覆盖任务，
每个任务同时记录需要扫描的频道及其对应区域。
若某个预计清除停留点能够覆盖一项搜索任务，就将该任务的频道扫描安排到这一停留点；
否则，为它保留独立测站。扫描是否完成，只在实际收到反馈后更新。

覆盖测站的位置也可以调整。在保留其分配区域全部位于可靠接收范围内的条件下，
移动测站以缩短前一访问点到该站、以及该站到后一访问点的总路程。
数值调整后再次检查覆盖条件，仅接受仍满足条件且路程缩短的位置。
因此，路线优化既调整访问顺序，也利用允许的测站移动范围减少绕行。

当机器狗已在某处停留时，还比较原地检测其他频道是否能节省后续行程。
例如，清除一个源后若能同时完成附近区域的频道扫描，就可以避免再次返回该区域。
这种复用节省的是移动，不同频道仍分别切换和检测。
扫描费用在路线预测中按每频道约 $6\,\mathrm s$ 估计，实际执行时依据当前频道计算切频次数。

每次反馈到来后，重新生成剩余任务并调整路线。
一次补测可能使某个源具备清除条件，一次无信号也可能使某个覆盖任务不再需要独立访问。
算法据此交替执行搜索、定位和清除，而不把三者分成相互独立的固定阶段。
