...
针对问题一，建立后续决策所需的几何基础，将示向约束转化为半平面交，证明有界定位多边形的直径可由顶点对取得，
并建立直径圆覆盖整个区域的充要判据。
两组构造算例的直径分别为 $20.00\,\mathrm m$ 与 $20.02\,\mathrm m$，覆盖结果却不同，
说明区域直径不能直接替代最小覆盖圆半径。

针对问题二，由首次观测更新位置分布，预测正常示向、距离过近及无信号三类反馈，以观测后的期望最小覆盖圆半径选择第二测点。
接收半径为 $1200\,\mathrm m$ 时，对称与不对称两组初始条件下的推荐点期望半径分别为
$23.15\,\mathrm m$ 和 $12.93\,\mathrm m$，较侧移 $300\,\mathrm m$ 策略分别降低 $68.2\%$ 和 $58.5\%$。
六组给定接收半径的加密复算中，原推荐点的目标值变化不超过 $1.3\%$。

针对问题三，将单次选点扩展为持续的贝叶斯更新与行动选择，以总虚拟时间评价检测、清除和覆盖搜索动作，
利用停留点复用减少专门绕行，并通过成功清除记录与频道排除证据判断任务完成。
一个本地案例清除全部15个源并确认无遗漏，总耗时 $2730.53\,\mathrm s$，移动距离 $10.143\,\mathrm{km}$。

针对问题四，将源类型、半径与朝向共同纳入贝叶斯更新，延续反馈预测与时间评价框架；
同时建立多点无信号联合排除判据，采用经3832个方格核验的21点布局保证空间覆盖。
16张随机地图与14张压力地图均完成全部搜索清除，平均总虚拟时间为 $5694.39\,\mathrm s$。
上述任务结果来自本地模拟，正式测试尚待完成。
...
依据上述完成判据，先统计清除比例和完整完成局数，再用平均定位清除时间及程序运行时间衡量效率。

图~\ref{fig:q3-execution-case} 给出一个本地模拟案例的实际执行过程。
算法清除全部 $15$ 个源并确认无遗漏，虚拟耗时 $2730.53\,\mathrm s$，移动距离 $10142.65\,\mathrm m$。
其中移动占总时间的 $74.3\%$，是该案例的主要耗时；末次清除后仍原地扫描 $30\,\mathrm s$，以完成剩余频道排查。
该案例用于展示轨迹与费用构成，正式测试结果及多局统计另行记录。
...
三次正式测试按表~\ref{tab:q3-official} 记录，数值待据原始日志填写，日志保留模拟器导出的文件名。
...
\begin{samepage}
这些指标反映实际任务的完成情况。根据上述本地案例并简单分析可知，单次任务中机器狗的移动耗时占比通常都最高。为进一步给移动耗时提供参照，我们尝试考虑极端情况下——源位置全部已知——需要走多远。
机器狗只需从原点出发访问所有源坐标且不必返回，属于开放式旅行商问题。
可以使用复杂度为 $O(N^2 2^N)$ 的动态规划求最短路程 $L_0$，以 $L_0/5$ 作为全知者的纯移动时间。
\par
\end{samepage}

值得一提的是，这一参照访问的是源坐标，而实际清除只需进入源周围 $20\,\mathrm m$，两者的访问要求有所不同。
若允许在清除圆盘内选点的最短路程为 $L_{20}$，则
\begin{equation}
  \max\{0,L_0-20(2N-1)\}\leq L_{20}\leq L_0.
  \label{eq:q3-oracle-bound}
\end{equation}
将圆盘内的访问点移至源坐标，首段至多增加 $20\,\mathrm m$，其余各段至多增加 $40\,\mathrm m$，即得左侧；
直接访问源坐标则给出右侧。

参照实验独立生成 $100000$ 张随机地图：源数 $N$ 在 $10$--$16$ 间等概率抽取，
给定 $N$ 后，各源独立、按面积均匀分布于半径 $1800\,\mathrm m$ 的圆域。
具体取 $r=1800\sqrt U$、$\theta=2\pi V$，其中 $U,V$ 为相互独立的 $[0,1)$ 均匀随机数。
这是用于评价的空间模型假设。全部地图均以精确动态规划求解，共包含 $1299298$ 个源，无求解失败；
仅计速度 $5\,\mathrm{m/s}$ 下的移动，不计检测、切频和清除操作。
按源数分组的结果见表~\ref{tab:q3-oracle-by-n}。

\begin{table}[!ht]
  \centering
  \caption{全知者逐点访问的十万图模拟结果（按源数分组）}
  \label{tab:q3-oracle-by-n}
  \small
  \renewcommand{\arraystretch}{1.15}
  \begin{tabularx}{0.96\linewidth}{c r >{\centering\arraybackslash}X >{\centering\arraybackslash}X}
    \toprule
    源数 $N$ & 地图数 & 平均最短路程/m & \shortstack{平均纯移动\\时间/(s/源)} \\
    \midrule
...
源数从 $10$ 增至 $16$ 时，平均最短路程由 $7831.68\,\mathrm m$ 增至 $9848.26\,\mathrm m$，
但每源分摊的移动时间由 $156.63$ 降至 $123.10\,\mathrm{s}/\text{源}$，说明比较耗时时需要考虑源数构成。
十万图的平均路程为 $8868.71\,\mathrm m$，平均纯移动时间为 $1773.74\,\mathrm s$/局。
记 $M=100000$，第 $k$ 张图的纯移动时间为 $T_k$，则逐局比值的平均
$\frac{1}{M}\sum_{k=1}^{M}T_k/N_k$ 为 $138.2281\,\mathrm{s}/\text{源}$，
蒙特卡洛标准误为 $0.05746\,\mathrm{s}/\text{源}$，均值的近似 $95\%$ 区间为
$[138.1155,138.3407]\,\mathrm{s}/\text{源}$。
该区间描述指定模型下均值的抽样误差，不是单张地图耗时的波动范围。

与任务统计一致的汇总口径为 $\sum_k T_k/\sum_k N_k$，其结果为 $136.5153\,\mathrm{s}/\text{源}$。
为考察源数分布的影响，另以已核验的 $152$ 局演练频率重新加权：
$N=10,\ldots,16$ 的频数依次为 $20,18,22,23,22,21,26$。
记表~\ref{tab:q3-oracle-by-n} 各组未舍入的平均秒/源为 $\bar t_n$，源数权重为 $p_n$，
则逐局等权指标估计为 $\sum_n p_n\bar t_n$，汇总指标估计为
$\sum_n p_n n\bar t_n/\sum_n p_n n$，结果见表~\ref{tab:q3-oracle-weights}。

\begin{table}[!ht]
  \centering
  \caption{源数权重与平均口径对全知者纯移动参照的影响}
  \label{tab:q3-oracle-weights}
  \small
  \renewcommand{\arraystretch}{1.2}
  \begin{tabularx}{0.96\linewidth}{X c c}
    \toprule
    统计方式 & \shortstack{逐局等权\\平均/(s/源)} & \shortstack{汇总平均\\/(s/源)} \\
    \midrule
    十万图直接统计 & 138.2281 & 136.5153 \\
    各源数按 $1/7$ 分层加权 & 138.1864 & 136.4781 \\
    按 $152$ 局演练源数频率加权 & 137.3258 & 135.6455 \\
    \bottomrule
  \end{tabularx}
  \par\smallskip
  \begin{minipage}{0.96\linewidth}\small
    注：三行均使用同一批随机地图。分层加权仅改变源数权重，位置仍来自面积均匀模型；
    直接统计与严格等概率加权的差别来自各源数实际抽样次数的波动。
  \end{minipage}
\end{table}

因此，在本模型下，即使预先知道全部源的位置，逐点访问仍需约 $135$--$138\,\mathrm{s}/\text{源}$ 的移动成本。
若加上每源一次光学精确定位及清除的 $5\,\mathrm s$，演练源数频率加权的汇总参照为
$140.6455\,\mathrm{s}/\text{源}$。
依据式~\eqref{eq:q3-oracle-bound}，十万图允许在 $20\,\mathrm m$ 内清除时的最优纯移动时间，
其逐局等权样本平均位于 $[130.5434,138.2281]\,\mathrm{s}/\text{源}$；
这是几何界限，尚未精确求解圆盘访问问题。
全知者参照省去了搜索、测向和无遗漏确认，且未与演练地图逐一配对，故只能比较总体量级，不能据此判断逐图最优性。
% 数据来源及数值复核：Q3/evidence/paper_q3_oracle/README.md。
% 历史 Bayes 演练50轮的版本与日志仍待核对，不作为当前策略的正文成绩。
\FloatBarrier
...
\subsubsection{类型与朝向未知时的贝叶斯更新及行动选择}

覆盖搜索负责发现源，发现后的任务是尽快缩小位置范围并完成清除。
正常示向仍按问题一构造楔形区域；计入两位小数的舍入后，楔形半角取 $1.005^\circ$。
“距离过近”和清除反馈分别按距离约束更新可行区域。
与此同时，无信号也参与定位，但必须结合此前记录判断它来自距离不足还是发射背面。

例如，以某候选源位置为原点，三次检测的相对位置与接收反馈如表~\ref{tab:q4-feedback-example}。
三条记录共同保留下来的解释是：源为定向源，朝向位于朝东的半周内，且 $1200\leq R_c<1300$。
可见，同样是无信号，第二条记录约束朝向，第三条记录约束半径；两者不能独立作圆域排除。

\begin{table}[htbp]
  \centering
  \caption{同一候选位置下，多次接收反馈对未知参数的共同约束}
  \label{tab:q4-feedback-example}
  \small
  \begin{tabularx}{\linewidth}{ccX}
    \toprule
    相对测点坐标/m & 接收反馈 & 结合此前记录得到的约束 \\
    \midrule
    $(1200,0)$ & 有信号 & $R_c\geq1200$；若为定向源，辐射范围须包含东方。\\
    $(-800,0)$ & 无信号 & 排除全向解释；西侧位于背面，朝向限于朝东半周（不含端点）。\\
    $(1300,0)$ & 无信号 & 东方已被确认处于辐射范围内，故须有 $R_c<1300$。\\
    \bottomrule
  \end{tabularx}
\end{table}

为比较不同候选位置，采用面积均匀的位置先验，并假设半径在 $[1000,1500]\,\mathrm m$ 内均匀分布，
全向与定向的先验概率各为 $1/2$，定向源朝向在整周内均匀分布；这些未知量初始独立，实际值在任务中固定。
对每个候选位置，累计所有能够解释历史反馈的类型、半径与朝向组合的先验概率，再计入示向读数的概率，
得到历史反馈在该位置下的似然 $q_\ell$。
将可行区域离散为面积为 $A_\ell$ 的网格单元后，位置权重为
\begin{equation}
  w_\ell=\frac{A_\ell q_\ell}{\sum_j A_jq_j}.
  \label{eq:q4-position-weight}
\end{equation}
权重表示各网格单元容纳源的相对可能性，用于加权预测下一测点的反馈。
计算使用随可行区域旋转的 $16\times16$ 网格，按裁剪后的面积赋权；
同一源的类型、半径和朝向由全部历史共同约束。

选点沿用问题三的候选动作与时间评分，但反馈预测须保留方向信息。
对于接近源、侧向交会及附近任务位置等候选点，利用历史记录约束后的类型、半径和朝向，
分别预测正常示向、距离过近与无信号的概率，按概率加权后续定位清除费用，
再加入到点移动、切频与检测时间。
因此，几何交会角较好的测点若很可能位于发射背面，也可能不如较近且易收到信号的测点。
预测示向先按 $10^\circ$ 分档筛选，再按 $2^\circ$ 分档细算；该分档仅用于动作评价，不改变实际测角误差。

随后，按式~\eqref{eq:q3-score} 将局部费用与其他源、剩余覆盖任务的访问费用合并，
选取下一动作，并在获得实际反馈后重新规划。
停留点复用须满足本问的联合覆盖条件，各频道仍逐一检测；固定节点的访问顺序求解与清除判据沿用问题三。
完整可行区域能被半径 $20\,\mathrm m$ 的圆覆盖时可保证清除，其他尝试则以成功反馈为准。
上述评分用于滚动选择动作，并不保证整场任务的全局最短时间。

当位置网格权重全为零时，先加密至 $32\times32$；仍无有效权重则保留几何可行区域，转入后备搜索，
不以数值权重为零判定源不存在。
已发现源可按边长 $20\,\mathrm m$ 的方格中心逐个尝试清除；未知频道继续覆盖检测，
必要时用 $700\,\mathrm m$ 方格网补充测点，并检查边界单元的顶点与联合覆盖条件。
曲线区域的近似仍遵循“可能区域外包、排除区域内缩”，避免误删真实位置。
当20个频道均已清除或被实际记录排除，或已成功清除16个不同频道的源时结束；
若先达到运行时限，则记为未完成。

\subsubsection{完整任务的实验结果}

本地实验考察两点：混合发射模式下能否完成全部搜索清除，以及完成任务需要多少时间。
采用上述21点布局和统一的定位、调度规则，在16张随机地图与14张压力地图上运行。
随机地图的源数为10至16，包含全向与定向源，接收半径在题设范围内变化；
...
不过，这些完成记录仅说明当前方案在所列案例中可执行，尚不能单独量化各算法组件的收益，
也不构成对所有地图均能在时限内完成的保证。
...
\subsection{几何判据与搜索完整性}

全文的贝叶斯决策依次涉及观测解释、反馈预测与行动评价，本节分别检查其几何依据、数值一致性和实际执行结果。
完整可行区域承接观测约束，概率权重用于比较候选动作，两者承担不同作用。
在题设误差界和接收机制成立时，几何更新应保留真实源位置；概率较小只能降低动作优先级，不能作为排除依据。
因此，位置积分无有效权重时采用加密或后备搜索，清除与频道排除则分别依赖实际成功反馈和完整覆盖证据。

问题一给出的顶点直径与覆盖判据无需对多边形内部采样。
表~\ref{tab:q1-experiment} 中两组近似等直径的构造区域具有不同覆盖结果，说明仅检查区域直径不足以保证清除，
还需检验完整区域能否被清除圆盘覆盖。
问题四的联合排除则同时要求凸包与距离条件：前者排除所有测点同处背面的可能，后者排除因距离过远而漏检的可能。
其独立覆盖检查包含目标域边界，3832个方格均通过，最大角点距离 $998.79\,\mathrm m$ 小于检查阈值 $999\,\mathrm m$。
该证据支持布局的空间覆盖性，任务在时限内能否完成仍需结合执行记录判断。

\subsection{数值离散与尺度一致性}

对问题二冻结的两组粗网格计算结果逐项复核，每组均含2821个候选测点。
各测点的正常示向、距离过近与无信号概率之和同1的偏差不超过 $1.2\times10^{-14}$，
说明该离散计算满足概率归一化要求。
对称情形共有1380对上下镜像测点，其目标值差异不超过 $3\times10^{-12}\,\mathrm m$，
与模型关于首示向线的对称性一致。

进一步的精度与参数检查见第~\ref{sec:q2-scale} 节。
位置积分、示向分档及圆弧近似共同加密后，六组原推荐点处的目标值变化均不超过 $1.3\%$，
支持这些固定测点的数值结果；该检查未重新证明连续域最优性，也未将三种离散误差分别估计。
当给定接收半径由1000增至 $1500\,\mathrm m$ 时，$J/R_0$ 均约为0.0193，
与首测点位于圆心、初始区域未受外边界裁剪时的近似尺度关系相符。
边界裁剪或先验改变后，应重新计算推荐区域。

\subsection{执行计时与特殊情形}

问题三本地案例的121次动作已有逐条反馈重放记录，计时也可由分项独立复核：
\[
  2028.53+99+525+75+3=2730.53\qquad(\mathrm s),
\]
五项依次为移动、切频、检测、成功清除及失败清除时间，与最终动作的累计耗时一致。
案例结束时，15个频道已清除，其余5个频道均有排除记录，因而完成判断包含了未知频道的排查。

问题四在随机地图之外加入边界朝外发射、切向发射、源聚集及极端或相关测角误差等压力情形，
所列30场均完整完成，说明当前方法在这些条件下具备执行可行性。
但有限压力测试不能替代先验敏感性分析，单例轨迹也不能给出总体性能结论。
问题三、四的场景不同，其总时间不作直接横向比较；正式测试结果仍按各问预留表格另行记录。
...
\textbf{定位结果与清除条件能够直接衔接。}
半平面约束将测角误差转化为可计算区域，顶点直径与最小覆盖圆分别描述不确定范围和保证覆盖所需的尺度。
由此，算法能够区分“估计位置附近可尝试清除”与“全部可行位置均可被清除”，避免用单个点估计代替完整定位区域。

\textbf{后验更新与行动评价形成统一过程。}
问题二由位置后验预测反馈，比较期望定位范围；问题三将同一过程扩展至预计完成时间，
问题四进一步纳入类型与朝向的不确定性。
路线随观测更新，清除停留点也可承担覆盖任务，使定位与搜索不必各自安排独立行程。
已有构造图说明了减少绕行的机制，实际案例则展示了该框架的执行过程。

\textbf{定向搜索具有明确的排除依据。}
多点联合排除无需预先识别源的朝向，覆盖布局又将局部判据扩展至全域。
搜索完成以实际检测证据为准，因此即使概率模型影响了选点顺序，也不会仅因某区域权重较小便提前宣告无源。

\subsection{局限性与改进方向}

首先，概率预测依赖位置先验、半径分布、类型比例及测角误差假设。
源聚集或误差相关会影响预测概率、检测顺序和预计耗时。
后续应在相同地图上改变先验与误差模型，比较完成率和耗时分布。
若误差超出预设上界，几何区域也可能误删真实位置，需要扩大误差集合或处理异常观测。

其次，有限候选点、网格积分和局部时间近似使整个策略仍属于启发式方法。
动态规划只能优化当前固定节点的访问顺序，不能保证后续信息未知时的整场最优。
可在近优候选区域内自适应加密，并在固定地图上分别关闭停留点复用、测站调整等组件，
比较各组件的实际收益，并用同图全知者参照衡量当前耗时与已知源位置情形的差距。

最后，模型忽略机器狗定位误差、转向加速及通行障碍，结果以本地模拟为主。
实际应用需采用可通行路径长度并计入定位误差，正式测试后还应结合耗时分布与失败案例评价可靠性。
