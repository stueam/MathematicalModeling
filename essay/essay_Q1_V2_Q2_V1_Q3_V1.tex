...
问题一采用以下假设：
\begin{enumerate}
  \item 各次有效示向观测对应同一静止干扰源，使同一位置能够同时满足全部观测约束。
  \item 检测点的平面坐标视为准确已知，暂不计坐标误差，其作用是确定各测向边界的起点。
\end{enumerate}
示向度误差不超过 $\pm1^\circ$ 是给定条件；问题一仅利用这一误差界，不预设误差的概率分布。
其他问题所需的附加假设在相应模型中说明。
...
在缺少其他位置信息的情况下，采用目标在 $\Omega$ 内\textbf{按单位面积均匀分布}的先验。
为构造示向度的似然函数，假设不同测点的测角误差相互独立，且在
$[-\varepsilon,\varepsilon]$ 内均匀分布。由此，各可行位置产生第一次示向度的
